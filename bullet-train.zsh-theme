...
prompt_git() {
  ...
  if $(git rev-parse --is-inside-work-tree >/dev/null 2>&1); then
    ...
    if [[ $BULLETTRAIN_GIT_EXTENDED == true ]]; then
      echo -n $(git_time_since_commit)  %{$fg[$BULLETTRAIN_GIT_FG]%}$(git_prompt_info)$(git_prompt_status)
    else
...
if [[ $BULLETTRAIN_PROMPT_SEPARATE_LINE == true ]]; then
  if [[ $BULLETTRAIN_PROMPT_ADD_NEWLINE == true ]]; then
    PROMPT='
%{%f%b%k%}$(build_prompt)
%{$fg[blue]%}%!%{$reset_color%} %{${fg_bold[default]}%}$(prompt_char) %{$reset_color%}'
  else
    PROMPT='%{%f%b%k%}$(build_prompt)
%{$fg[blue]%}%!%{$reset_color%} %{${fg_bold[default]}%}$(prompt_char) %{$reset_color%}'
  fi
else
  PROMPT='%{%f%b%k%}$(build_prompt)%{$fg[blue]%}%!%{$reset_color%} %{${fg_bold[default]}%} $(prompt_char) %{$reset_color%}'
fi
